...
function formatLastActiveTooltip(lastActiveMs: number | null, nowMs: number): string {
...
function getStatusField(status: PresenceStatus | null, label: string): string | null {
  if (!status) return null
...
function parseLocalAgentStatus(text: string): PresenceStatus | null {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      const now = Date.now()
      const statusValue = typeof parsed.status === "string" ? parsed.status : "active"
      ...
      return {
        ...
        activeEditor: typeof parsed.activeEditor === "string" ? parsed.activeEditor : undefined,
        project:
        ...
        totalActiveMs:
          typeof parsed.totalActiveMs === "number" || typeof parsed.totalActiveMs === "string"
        ...
        timestamp:
          typeof parsed.timestamp === "number" || typeof parsed.timestamp === "string"
  ...
  return {
    ...
    activeSince:
      typeof activeDurationMs === "number" && activeDurationMs < now ? now - activeDurationMs : undefined,
    lastSeen: now,
  }
}
...
function getProjectDetails(status: PresenceStatus | null): {
  ...
  const extractProjectFromRichText = (value: string): string | null => {
    ...
    const project = match[1].trim().split(/[\\/]/).filter(Boolean).pop() || null
  ...
  const cleanCandidate = (value: string) => {
    ...
    const parts = trimmed
      ...
      .filter(Boolean)
      .filter(
        (part) => !/^(visual studio code|vs code|vscode|cursor|code)$/i.test(part)
      )

    const likelyProject =
      parts.find((part) => !/\.[a-z0-9]{1,5}$/i.test(part)) || parts[0] || trimmed

    const basename = likelyProject.split(/[\\/]/).filter(Boolean).pop() || likelyProject
    if (/^(idle|active|online|offline|away|afk|dev presence)$/i.test(basename)) {
      return null
    }
...
function getEditorIcon(editorName: string | null): { src: string; alt: string } {
...
export default function DevPresence() {
  ...
  useEffect(() => {
    ...
    const isWeakPayload = (data: PresenceStatus) => {
      ...
      const hasInlineDetails = /project:|editor:|file:|agent:/i.test(statusString)
    ...
    const shouldTreatAsCoding = (data: PresenceStatus, currentTime: number) => {
      ...
      const seenRecently = typeof seen === "number" ? currentTime - seen < 300_000 : false
      return !explicitIdle && (explicitCoding || Boolean(data.isActive) || seenRecently)
    }
  ...
  const fromDuration =
    typeof now === "number" && typeof payloadDurationMs === "number" ? now - payloadDurationMs : null
  ...
  const hasFileContext =
    ...
    (typeof status?.language === "string" && status.language.trim().length > 0) ||
    Boolean(statusFile) ||
    Boolean(statusLanguage)
  const isThinkingMode = isCoding && Boolean(editorName) && !hasRealProject && !hasFileContext
          ...
          {isThinkingMode ? (
            ...
            <>
              ...
              {hasRealProject && projectName ? (
                ...
                <strong className="font-semibold text-text-primary">current workspace</strong>
              )}
              {formattedEditorName ? ` in ${formattedEditorName}` : ""}{" "}
              for {formatDuration(codingDuration)}
            </>
          )}
        </span>
